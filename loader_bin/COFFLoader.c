...
int go(void) {
    ...
    arguments = unhexlify((unsigned char*)argv[3], &argumentSize);
        ...
        // Find the magic header 0x90e63f1c in memory
        while (1) {
            if (seek_offset[0] == 0x1c && seek_offset[1] == 0x3f && seek_offset[2] == 0xe6 && seek_offset[3] == 0x90) {
                break;
            }
            seek_offset += 1;
        }
		...
		seek_offset = (unsigned char*) go;
        // Find the magic header 0x80e63f1c in memory
        while (1) {
            if (seek_offset[0] == 0x1c && seek_offset[1] == 0x3f && seek_offset[2] == 0xe6 && seek_offset[3] == 0x80) {
                break;
            }
            seek_offset += 1;
        }
        argumentSize = *(uint32_t*)(seek_offset+4);
        arguments = seek_offset+8;
